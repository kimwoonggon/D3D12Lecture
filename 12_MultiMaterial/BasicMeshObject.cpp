...
void CBasicMeshObject::Draw(ID3D12GraphicsCommandList* pCommandList, const XMMATRIX* pMatWorld)
{
	...
	// box냐 quad냐에 따라서 6,1,6,1 (m_dwiTriGroupCount 변화)
	...
	* Upload Heap (ID3D12Resource)
		┌──────────┬──────────┬──────────┬─ ─ ─┐
		│  CB[0]   │  CB[1]   │  CB[2]   │ ... │  ← pSystemMemAddr로 CPU에서 직접 write
		└──────────┴──────────┴──────────┴─ ─ ─┘
		...
		└──────────┴──────────┴──────────┴─ ─ ─┘
		- pSystemMemAddr	CPU에서 상수값(행렬 등)을 쓰는 주소
		- pGPUMemAddr	GPU Virtual Address (Upload Heap의 GPU 주소)
	...
	*/
	...
	/*
	* CPU handle로 내용을 쓰면 GPU handle도 자동으로 같은 슬롯을 바라보므로, 
	  별도로 GPU handle을 세팅할 필요가 없고, GPU handle은 그냥 "어디서부터 읽을지"의 시작 주소 역할만 합니다.
	*/
	...
	// per tri-group
	for (DWORD i = 0; i < m_dwTriGroupCount; i++) // 6, 6, 1 (box, box, quad)
	{
	...
	for (DWORD i = 0; i < m_dwTriGroupCount; i++)
	{
		...
		/*
		 ...
		 rootParameters[1].InitAsDescriptorTable(...rangesPerTriGroup...);   // RootParam[1] = SRV 1개짜리 테이블
		*/
